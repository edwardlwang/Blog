...
tags: [Algorithms, Back Tracking Method]
...
```cpp
vector<vector<int>> result;   // 保存所有可行解

void backtrack(vector<int>& path, vector<int>& choices) {
    if (满足结束条件) {
        result.push_back(path);   // 注意：不能直接保存引用
        return;
    }
...
void backtrack(vector<int>& path, vector<vector<int>>& result) {
    if (is_goal(path)) {
        result.push_back(path);   // 拷贝路径
        return;
    }

    for (int choice : get_choices(path)) {
        if (!is_valid(choice, path)) {
            continue;
        }
...
- 递归前“做选择”，递归后“撤销选择”。
- 保存结果时要复制当前路径，例如 `result.push_back(path)`。
...
含义是：还需要选 `k - path.size()` 个数，当前最多只能选到 `n - (k - path.size()) + 1`，否则剩余数量不够。
...
public:
    vector<vector<string>> partition(string s) {
        ...
        function<bool(int, int)> is_palindrome = [&](int left, int right) {
            while (left < right) {
                if (s[left] != s[right]) {
                    return false;
                }
...
public:
    void solveSudoku(vector<vector<char>>& board) {
        function<bool(int, int, char)> is_valid = [&](int row, int col, char num) {
            for (int i = 0; i < 9; ++i) {
...
```cpp
result.push_back(path); // vector 会拷贝内容，但若 path 是指针或引用则不行
```
